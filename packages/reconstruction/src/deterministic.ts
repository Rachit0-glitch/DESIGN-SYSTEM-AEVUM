...
function uuidFromValue(value: unknown): string {
  const hex = fingerprint(value).slice("sha256:".length);
...
export function deterministicCommandId(value: unknown): string {
  return `cmd_${uuidFromValue(value)}`;
}

export function deterministicTransactionId(value: unknown): string {
  return `tx_${uuidFromValue(value)}`;
}

export function deterministicScopedId(scope: string, value: unknown): string {
  return `${scope}:${fingerprint(value).slice("sha256:".length, "sha256:".length + 32)}`;
}
