...
export function threeFingerprint(value: unknown): `sha256:${string}` {
  return `sha256:${createHash("sha256").update(stableSerialize(value)).digest("hex")}`;
}

export function deterministicEntityId(prefix: string, scope: unknown): string {
  const hex = createHash("sha256").update(stableSerialize({ prefix, scope })).digest("hex").slice(0, 32).split("");
  hex[12] = "5";
